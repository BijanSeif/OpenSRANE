...
class TankHoleInitRate(_NewClass,_GlobalParameters):
    
    '''
    OutFlow Model with constant rate equal to initial rate For Liquid Or Gas OutFlow From a Tank
    
    Calcaulations is according Casal book But without considering outflow rate variations
    
    '''
    
    Title='TankHoleInitRate'
    
    def __init__(self,tag,Hole_Diameter=0.01,Hole_Height_FromBot=0,delta_t=0.1, Cd=1):
        ...
        self.name=f'TankHoleInitRate with Diameter= {Hole_Diameter} and with Height(From Bot)= {Hole_Height_FromBot} and dt= {delta_t} and Cd= {Cd} '
    ...
    def Calculate(self):
        ...
        if h<=0:    #The condition that Level of hole be higher than level of liquid or be equal
            ...
            warning(f'(TankHoleInitRate) For Plant Unit with tag={UnitObject.tag} OutFlow is not calculated Because h={Vsubs/(_math.pi*d**2/4)}<Hole_Height_FromBot={self.Hole_Height_FromBot} and results = [0]')
            ...
            warning(f"Error: (TankHoleDuration) in unit with tag:{UnitObject.tag} Internal Pressure is less than -Rho*g*h+Pout that cause negetive value under" +
            ...
            #initial outflow rate 
            mdot=Aor*Rho*Cd*_math.sqrt(2*((Pin-Pout)/Rho+g*h))
            
            while V<V0 and mdot>=0:
            ...
            if V>V0 and TotalMass_Release[-1]<V0*Rho:
