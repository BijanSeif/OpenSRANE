...
class TankHole(_NewClass,_GlobalParameters):
    ...
    def __init__(self,tag,Hole_Diameter=0.01,Hole_Height_FromBot=0,delta_t=50, Cd=1):
        ...
        self.name=f'TankHole with Diameter= {Hole_Diameter} and with Height(From Bot)= {Hole_Height_FromBot} and dt= {delta_t} and Cd= {Cd} '
    ...
    def Calculate(self):
        ...
        if Vsubs==None:
            warning (f'(TankHole outflow model) is not calculated for unit Object with tag {UnitObject.tag} because no dimension or Substance volume (V_subs) is defined for mentioned plant unit')
            return -1
        ...
        if h<=0:    #The condition that Level of hole be higher than level of liquid or be equal
            ...
            warning(f"(TankHole) Error: in unit with tag:{UnitObject.tag} Internal Pressure is less than -Rho*g*h+Pout that cause negetive value under" +
