...
class TankHoleDuration(_NewClass,_GlobalParameters):
    
    '''
    Tankhole for specific duration
    OutFlow Model For Liquid Or Gas OutFlow From a Tank But the
    ...
    '''
    Title='TankHoleDuration'
    
    def __init__(self,tag,Hole_Diameter=0.01,Hole_Height_FromBot=0,delta_t=50, Cd=1, Duration=1000):
        ...
        self.name=f'TankHoleDuration with Diameter= {Hole_Diameter} and with Height(From Bot)= {Hole_Height_FromBot} and dt= {delta_t} and Cd= {Cd} '
    ...
    def Calculate(self):
        ...
        if h<=0:    #The condition that Level of hole be higher than level of liquid or be equal
            ...
            warning(f'(TankHoleDuration) For Plant Unit with tag={UnitObject.tag} OutFlow is not calculated Because h={Vsubs/(_math.pi*d**2/4)}<Hole_Height_FromBot={self.Hole_Height_FromBot} and results = [0]')
            ...
            mdot=0
            dm=0
            while V<=V0 and mdot>=0 and t<=Duration+dt:
